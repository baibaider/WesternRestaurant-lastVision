...
function renew() {
    $.ajax({
        type :"GET",
        async : true,
        url : "https://easy-mock.com/mock/5a17be8bdc09a2675175fc7b/zyf/?jsonp_param_name=callback",
        dataType : "jsonp",
...
function toL(img_obj){
    if(i == $(img_obj.children("a")).length/2){
        i = 0;
        img_obj.css("left",0);
    }
    i++;
...
function toL1(img_obj){
    if(j == $(img_obj.children("a")).length/2){
        j = 0;
        img_obj.css("left",0);
    }
    j++;
